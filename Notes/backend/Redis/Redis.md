...
### 查看当前库的所有key
Redis中有16个库, 默认使用的是0号库, 我们通过下面的命令查看当前库中有哪些key, 相当于 keys()
...
# Reids类型: 列表 List
List是单键多值, Redis还是key-value的形式, 我们的key还是String类型, 但是一个key对应着多个值 如
...
### 添加操作: 一次性的向hash中添加多个 field : value
...
### 添加操作: 当hash中没有指定的field的时候再添加  field : value
```s
...
**它的每个成员都关联了一个评分(score)**, 这个评分被用来按照最低分到最高分的方式排序集合中的成员
