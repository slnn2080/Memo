...
**2. 开启事务的注解驱动**  
**作用:**  
- 将事务管理器(切面)中的通知作用到当前的连接点 (``@Transactional``加到了哪个方法上 哪一个方法就是连接点)
...
这是事务传播行为的一种设置, propagation属性定义了方法执行时所处的事务上下文中的行为。

**常见的取值包括:**
1. REQUIRED(默认值): 它表示如果当前存在事务, 则加入该事务, 如果没有事务, 则新建一个事务
```s
这是最常用的事务传播行为, **它确保方法总是在一个事务内运行**, 如果当前没有事务, 则创建一个新事务
```

2. REQUIRES_NEW: 创建一个新的事务，并且暂停当前正在进行的事务（如果存在）
3. SUPPORTS: 如果当前存在事务，则加入该事务；如果当前没有事务，则以非事务的方式执行。
4. NOT_SUPPORTED: 以非事务的方式执行，并且暂停当前正在进行的事务（如果存在）
5. MANDATORY: 要求当前存在事务，如果不存在则抛出异常
6. NEVER: 要求当前没有事务，如果存在则抛出异常
7. 如果当前存在事务，则在嵌套事务内执行；如果当前没有事务，则创建一个新的事务
...
指定事务的隔离级别，用于控制多个事务同时执行时的数据隔离程度

**常见的取值包括:**
1. DEFAULT: 由底层数据源决定隔离级别
2. READ_UNCOMMITTED: 允许事务读取未提交的数据，最低的隔离级别，会导致脏读、不可重复读和幻读问题
3. READ_COMMITTED: 只允许事务读取已提交的数据，避免了脏读，但是仍可能出现不可重复读和幻读问题
4. REPEATABLE_READ: 确保在事务执行期间，同一数据项的多次读取结果是一致的，避免了不可重复读和脏读，但是仍可能出现幻读问题
5. SERIALIZABLE: 最高的隔离级别，确保事务之间的操作串行执行，避免了所有的并发问题，但性能较差
...
我们可以通过注解设置回滚策略, 以下它的属性名可选值为: 用于配置在何种异常情况下回滚事务或不回滚事务 
```s
