...
也就是说我们的 外层应用程序必须通过 shell 才能嘈杂哦Linux内核 从而控制硬件
...
### shell作用:
shell是一个命令行解析器, 它接收应用程序/用户命令, 人那后调用操作系统内核
...
### Shell脚本声明格式
以下面的#开头, bash的部分是指定解析器, #不是注释它是命令
```shell
#!/bin/bash
```
...
**原因:**  
我们使用 sh 或者 bash 来执行脚本的时候 是由bash解析器来执行的 所以脚本本身不需要执行权限
...
## 特殊变量: $n
使用场景 在我们使用 ``sh 脚本.sh`` 的时候可以往脚本中传递参数, $1就是第一个我们传入的参数
...
我们在指定脚本的时候 会给脚本文件传入参数, 它的作用就是**获取所有输入参数个数**  
