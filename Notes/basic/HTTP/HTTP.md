...
- 以往的认知是 get 和 post 之前请求是有区别的 比如如下等等
<!-- 
...
### Get请求怎么携带请求体参数
- http协议是支持get请求可以携带请求体的 后端也可以接收请求体参数
<!-- 
  有些后台服务器不支持 使用 get 的使用 携带请求体参数
 -->

- 但是 浏览器会限制get请求不携带请求体
- 也就是说
 - 1. 要是想通过 浏览器发送 get请求的时候 携带 body参数 那么需要后台下载相关的依赖
- 比如java中是使用了 httpclient 依赖

- 2. 后台使用 axios 发送 get请求的时候 携带body参数是可以接收到的
```js 
// server1
...
### Get 请求时 url 参数长度限制
- 1. http中 get请求提交的数据大小长度并没有限制 同时 http规范没有对url长度进行限制
- 目前说得 get 长度有限制 是特定的浏览器 以及 服务器对它的限制
- 各种浏览器 和 服务器的最大处理能力如下
