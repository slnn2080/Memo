...
```scss
// 1. 遍历是为了将列表中的元素一个个的取出来使用, 所以列表中有几个元素 它就会执行即便 { ... } 中的逻辑

// 2. 遍历的时候会依次从 列表中获取元素 获取到的元素会保存在 $el 变量中 供我们在 {} 中使用 
@each $el in $bg-color-list {
...
我还是喜欢叫它对象, 后面我就不说是map了 我直接说对象了

首先我们先回顾一下 list, 上面说了 list 是一种存放数据的容器 它有什么样的特点呢

1. list容器中存放的数据 是有顺序
2. list容器中的元素都对应有一个下标值, 我们是通过下标值来找到容器中的元素的, 也就是说 ``下标 <-> 数据``

<br>

我们的对象类型, 它也是一种数据的容器, 不同的是 它是通过 ``key: value`` 这种形式保存数据的, 一个key对应着一个value, 这里十分像我们在写样式 ``color: #c2185b``
...
我们使用单一的值也能定义出来描述学校的变量信息, 但是 值和值 之间没有联系, 现在我们想描述是学校的信息, 不然学校名也好 还是地址 还是描述, 它们应该是一个整个, 而不是一个一个不相关的变量, 也就是说 我们的 name address desc 都应该作为学校这个事物身上的属性出现

而不是上面分别定义了三个变量来描述学校, 那怎么将零散的变量让它们关联起来成为一个一体呢? 很简单 你现在不是零散的么, 我给你们这些零散的东西 装到一个塑料袋里 是不是就是一体的了
