...
# 自我简介: 
hello, 大家好, 大家可以叫我 jiangjiangjiangjiang  
在未来的一段时间里 我给大家带来一套scss教学视频 希望大家能够喜欢  

网上的scss视频不少 你说为什么我又出一套   
要知道一道菜不可能满足所有人的胃口 萝卜青菜各有所爱 兴许我就是你的菜呢 是不

so 接下来的一段时间里 我会让大家喜欢上我 附带着给大家讲解下scss
...
是的 一般都是UP教框架的最后 会整合一套实战教程是么 我这小小的一css预处理器 也来一套实战 目的就是在交给大家基础知识的同时 也希望通过敲代码来巩固前面所学到的知识 同时告诉大家scss在实战方面的一些使用技巧
...
另外关于这套课程的受众人群 嗯... 
我希望是 只学习了html css的小伙伴们也能听懂
所以在整套scss中 讲到类似 if for 语句的地方 可能会稍微说的多一些 会的小伙伴呢 也别跳过 别嫌我啰嗦哈
...
- 已经工作的小伙伴们 在接触公司项目的时候 发现项目里面使用的scss
- 在自学的小伙伴 跟着视频学习的时候 可能接触到了 scss

不管是哪种情况 在大家去搜scss的相关资料的时候 可能会搜到 sass 和 scss 相关字样, 当搜到它们的时候有没有蒙圈的, 会不会有它俩是个啥? 嘛关系? 学哪个？  

<br>

这节课带大家简单的了解一下 sass 同时也说说 sass 和 scss 之间的关系

说到 sass 那就简单的提下 haml  
haml 和 sass 之间的关系 就相当于html 和 css  
html 负责页面的结构, css 负责页面的样式  haml 和 css也是一样

haml 是 html 的模版引擎 咱们就简单的理解为 简写html的方式
sass 则是 css 的预处理语言 咱们就简单的理解为 让css写起来结构更清晰 更舒服 同时扩展了css的种种功能

其实不管是 haml 还是 sass 初衷都是为了偷懒 说白了就是懒 想少写代码

当时吧就有一群大大他们觉得写 html 太麻烦了 又尖括号 要开始又要管闭合 费劲

既然这么费劲 他们就想要是能怎么舒服怎么来多好 于是呢 他们就按照自己的编码习惯 开发一门语言 就是haml ok html的问题解决了 还有css啊 所以同样风格的可以简写css的语言也出来了 就是sass

我们简单的体会下 haml 的书写格式 不是真写哈 咱就看看 haml 样子  
因为要写 haml 还需要进行些 配置 咱们又不专门讲它 没必要 我写写 大家看看差别就可以了
...
怎么样 除了看的不太习惯 确实让html代码简洁了很多 其实要真写多了你也会觉得haml写起来很舒服 有机会的话 我会给大家另开一套 pug 的课程 也很有意思
...
有没有发现 简洁了很多 这种缩进的形式 显得结构也清晰了很多 
其实对于我们写css写久了的人来说 还是有点别扭 没括号 没分号啊 感觉还是有点生

因为这 sass 又想了 为了让熟悉css的人能够更加的平稳的过渡到 sass

在sass 3.0之后将 将 .sass 结尾的文件 改成 .scss 结尾

那有人说了 哥们就改了一个后缀名么？ 不是 语法规则也不一样了 它把括号 和 ； 给你加回来了 这样就熟悉了吧
...
它额外的提供了很多编程语言才有的特性 比如变量 函数 逻辑控制等等 让css的功能更加的强大了 

但是用scss写出来的代码 浏览器不认识啊 浏览器只认识css啊 所以我们写完的scss文件还要编译成css文件 后才能被浏览器所识别
