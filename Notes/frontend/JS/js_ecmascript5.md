...
### 扩展: 中括号的含义
![对象的中括号写法含义](./images/对象的中括号写法含义.png)

<br>

属性名的类型一种就2种 string | symbol
...
console.log(arr[0], arr['0'])  // 3 3
```

<br>

上面的 [] 不是数组特有的 表达从数组中取索引位置的值, 它的含义是通过 ``对象[属性名]`` 这种格式取对应的值

中括号中传入的是属性的名字, 属性的名字可以是字面量 ``obj['name']``, 中括号还可以传递变量 ``obj[a]``

<br>

如果我们传入的是字面量 而且该字面量的值是满足标识符规范的 这时就可以写成 obj.name 的格式

数组也可以使用这种方式 arr[0] 是因为数组本身也是对象, 我们传入的number 0会被转换为字符串
```js
arr: {
  0: 1,
  length: 1
