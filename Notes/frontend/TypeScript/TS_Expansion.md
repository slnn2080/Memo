...
**为什么 我给i对象设置了一个属性a会报错呢? 为什么要在i对象中将b和c属性写上之后才不报错呢? 不是交集么?交集不是两个集合中共同部分算交集, 我只声明a属性应该是对的呀**

<br>

联合类型 和 交叉类型 确实是我们说的 并集 和 交集 **但是 并的 和 交的 并不是属性字段**
...
### 解析交集: 
ts中的交集指的是两个对象的值, 比如我们有两群人, 一群是坤坤粉丝 一群是马老师粉丝

坤粉都有一些共同的特点 唱歌 跳舞 打篮球, 它们相当于属性的字段

坤粉都有一些共同的特点 五连鞭 太极 讲武德, 它们相当于属性的字段
```s
坤粉
  唱歌
  跳舞
...
**交集:**  
交集的意思不是说 将 ``唱歌 跳舞 打篮球`` 和 ``五连鞭 太极 讲武德`` 这些属性字段 取交集

而是坤坤粉丝那群人 和 马老师粉丝那群人 有相交的地方 指的是 **值的相交**

比如说房间里有一群人 我说坤坤粉丝举左手 马老师的粉丝举右手 什么叫做交集? **两只手都举起来的叫交集**

当我们将两只手都举起来后 就同时具备了 ``唱歌 跳舞 打篮球`` 和 ``五连鞭 太极 讲武德`` 这些能力

**这就是两个对象一求交集 所有的属性都要写全**

<br>

**并集:**  
坤坤的粉丝举左手, 马老师的粉丝举右手, 凡是举了手的都是并集
...
我们可以在 u 对象中声明 a 和 b, 也可以声明b 和 c, 怎么写字段都没有问题, 但是为什么读属性 u.c 的时候会报错呢?

因为只要我们举了手就属于并集中的情况 但是不是说有可能出现要求 坤粉来套五连鞭? 坤坤的粉丝不会五连鞭

也就是说 **使用对象并集的时候 我们只能使用两个对象中 相同的字段**
...
### 举例:
```js
// test1 为 a, 因为 既要满足string 又要满足a的 只能是a
type test1 = 'a' & string
