...
### 计算属性的配置项:
在Vue 3中，计算属性默认是启用缓存的，这是因为缓存可以提高计算属性的性能，避免不必要的重复计算。

如果你需要在计算属性中关闭缓存，你可以使用Vue 3中新增的一个选项：cache。将cache选项设置为false将禁用计算属性的缓存，每次访问计算属性都会重新计算它的值。
...
## **自定义的hooks**
自定义hook 就是一个函数 我们定义一个函数并返回 同时hooks文件中就相当于setup中写逻辑一样 使用什么引入什么就可以
