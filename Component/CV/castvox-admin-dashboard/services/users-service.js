...
function get(id, axios) {
  return axios.get(`${url}/users/${id}?include=roles`).then((response) => {
...
function add(user, axios) {
  ...
  return axios.post(`${url}/users?include=roles`, payload).then((response) => {
...
function update(user, axios) {
  ...
  return axios
    .patch(`${url}/users/${user.id}?include=roles`, payload)
    .then((response) => {
      return jsona.deserialize(response.data);
    });
}
